...
public class NutritionMonitor implements MonitoringStrategy {
	...
	static org.apache.log4j.Logger log = Logger
			.getLogger(NutritionMonitor.class.getName());
	 ...
	 * @param a
	 * @throws SQLException
	 *             This method is to keep the format of MonitoringStrategy
	 *             interface.
	 */
	 ...
	 * 
	 * @param ObjFoodLog
	 *            This method is used to monitor the quantity of food component
	 *            in a a daytime period, if a particular ingredient cross daily
	 *            recommended limit...it will intimate to wellness services by
	 *            absorbing the service.
	 */
	public void doMonitor(FoodLog ObjFoodLog) {
		...
		try {
			...
			cstmt = conn.prepareCall("{call usp_NutiritionMonitorvdemo( " + a
					+ "," + s + "," + "?,?,?,?,?)}");
			cstmt.setInt(1, 5);
			...
			if (cstmt.getInt(1) == 0) {
				...
				System.out.println(" Plz intimate the SCL for Recomendation"
						+ cstmt.getInt(1));
				objRestServiceNotification = new RestServiceNotification();
				// communicate the information to the SCL
				// objRestServiceNotification.notifyToRestService(getNotificationMessage(stMapId,
				// stUserId, stConsumeFat),
				// "http://163.180.116.185:8080/SCLMiningMind2.5/webresources/SCLService/PushPARecommendation");
				System.out
						.println(" *******************************************");
			} else {
				System.out.println(" Sorry very wrong Information"
						+ cstmt.getInt(1));
			}
	 ...
	 * 
	 * @param ObjFoodLog
	 * @return This method is used to log the food information, whenever user
	 *         take food and want to log it for recording and logging.
	 */
		...
		try {
			...
			ClientResponse serverResponse = srv
					.type(MediaType.APPLICATION_JSON).post(
							ClientResponse.class, ObjFoodLog);
	 ...
	 * @param UserId
	 * @param ConsumedFat
	 * @return This method is used to build notification message when a user's
	 *         food particular ingredients cross a threshold value.
	 */

	protected SituationNotification getNotificationMessage(String MapId,
			String UserId, String ConsumedFat) {
		Connection conn = null;
		Statement sta = null;
		...
		try {
			CallableStatement callableStatement = conn
					.prepareCall("{call usp_NutritionMonitoringEvents(" + MapId
							+ ")}");
			ResultSet rs = callableStatement.executeQuery();
			if (rs != null) {
				while (rs.next()) {
					String stActivity = rs.getString("Activity");
					String stActivityOperator = rs
							.getString("ActivityOperator");
					String stActivityValue = rs.getString("Activityvalue");
					String stActivityDataType = rs
							.getString("ActivityDataType");
					String stMeasuringMetric = rs.getString("MeasuringMetric");
					String stMeasuringOperator = rs
							.getString("MeasuringOperator");
					String stMeasuringTargetValue = rs
							.getString("MeasuringTargetValue");
					String stMeasuringDataType = rs
							.getString("MeasuringDataType");

					objSituationLog.setUserID(UserId);
					objSituationLog.setSituationCategoryID("16");
					objSituationLog.setSituationDescription("Fat "
							+ stMeasuringTargetValue);
					...
					System.out
							.println("\t Current Activity:"
									+ objSituationConditionsMonitor
											.getConditionValue());
					System.out.println("\t Consumed Fat:"
							+ objSituationConditionsMeasuring
									.getConditionValue());
					System.out
							.println("*****************************************\n");
					System.out
							.println("****** SCL Recomendation Response *******");

					objListSituationConditions
							.add(objSituationConditionsMonitor);
					objListSituationConditions
							.add(objSituationConditionsMeasuring);

				}// end of loop
			}
			// /////////////stored procedure////////////
			CallableStatement callableStatement1 = conn
					.prepareCall("{call usp_NutritionSituationConstraints("
							+ MapId + ")}");
			...
			if (rs1 != null) {
				while (rs1.next()) {
					...
					String stConstraintOperator = rs1
							.getString("ConstraintOperator");
					String stConstraintValue = rs1.getString("ConstraintValue");
					String stConstraintDataType = rs1
							.getString("ConstraintDataType");
					System.out.println(stConstraintKey + " "
							+ stConstraintOperator + " " + stConstraintValue
							+ " " + stConstraintDataType + " ");
					...
					System.out
							.println("********** Situation Constrainds **********");
					System.out.println("\t Mapping ID:" + situationLogID);
					System.out.println("\t User Id:" + UserId);
					System.out.println("\t Situation Details:");
					System.out.println("\t Activity Constraint:"
							+ objSituationConditionsConstraints
									.getConditionKey());
					System.out.println("\t Activity Constraint Value:"
							+ objSituationConditionsConstraints
									.getConditionValue());
					System.out
							.println("*****************************************\n");
	 ...
	 * @return This method is used to get the logID which is required by SCL for
	 *         reasoning to obtain the appropriate recommendation.
	 */
	protected static String getSituationLog(SituationLog objSituationLog) {
		String situationLogID = null;
		try {
			ClientConfig config = new DefaultClientConfig();
			config.getClasses().add(
